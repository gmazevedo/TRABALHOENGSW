...
export class ProfileComponent implements OnInit {
  ...
  public userDisplayCols = ['Nome', 'Email', 'Ocupante'];
    ...
    this.vacanciesInterestsService.getUpdateListener().subscribe({
      next: (res) => {
        res.data.forEach((interest) => {
          this.interestsMap.set(
            interest.vacancy_id,
            interest.registration_numbers
          );
        });
      },
    });
  ...
  public async saveForm() {
    try {
      ...
      this.getAreaOptions().controls.forEach((ct, index) => {
        if (
          ct.value &&
          !this.currentUser.area_interests.includes(this.areaOptions[index])
        ) {
  ...
  public async saveVacancyForm() {
    try {
      ...
      this.getVacancyAreaOptions().controls.forEach((ct, index) => {
        if (
          ct.value &&
          !this.editingVacancy.areas.includes(this.areaOptions[index])
        ) {
          areasToInsert.push(this.areaOptions[index]);
        } else if (
          !ct.value &&
      ...
      if (areasToDelete.length > 0) {
        await this.vacanciesService.deleteVacancyAreas(
  ...
  public isOccupant(element: User): boolean {
    return (
      this.editingVacancy &&
      this.editingVacancy.occupant_registration_number &&
      this.editingVacancy.occupant_registration_number ===
  ...
  public onChooseStudent(element: User) {
    try {
      this.vacanciesService.updateOccupant(
        this.editingVacancy.vacancy_id,
        element.registration_number
      );
      this.resetVacancyForm();
  ...
  public getInterestedUsers() {
    return this.editingVacancy &&
      this.interestsMap.get(this.editingVacancy.vacancy_id)
      ? this.users.filter((user) =>
          this.interestsMap
            .get(this.editingVacancy.vacancy_id)
            .includes(user.registration_number)
        )
      : [];
  }
